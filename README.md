...
- `packages/eslint-plugin-sweepit` - The rule engine. This package contains Sweepit's opinionated ESLint rules and shared flat configs (`core`, `react`) used to enforce guardrails, including `typescript-eslint` `recommended` and `recommendedTypeChecked` flat defaults in the core baseline.
