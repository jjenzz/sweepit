 ...
 */
function getVerbSuffixSuggestion(
	propName: string,
	verbs: string[],
): string | null {
...
const rule: Rule.RuleModule = {
	meta: {
		...
		docs: {
			description:
				"Ensure on* handler prop names end with a verb (for example onValueChange).",
			url: "https://github.com/jjenzz/sweepit/tree/main/packages/eslint-plugin-sweepit/docs/rules/jsx-on-handler-verb-suffix.md",
		},
		messages: {
			mustEndWithVerb:
				"Handler prop '{{prop}}' should end with a verb (for example 'onValueChange'). See: https://github.com/jjenzz/sweepit/tree/main/packages/eslint-plugin-sweepit/docs/rules/jsx-on-handler-verb-suffix.md.",
			preferVerbSuffix:
